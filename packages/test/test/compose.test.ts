...
import { ComposeHandler } from "../src/compose.ts";

describe("unit-ComposeHandler", () => {
	it("throws an error if the given compose command doesn't exist", async () => {
		const c = new ComposeHandler({
			composeCommand: "im not a command",
			testName: "unit-ComposeHandler",
		});
		expect(c.precheck).toThrow();
	});
});
